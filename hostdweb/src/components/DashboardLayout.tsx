...
export default function DashboardLayout({ children, currentPage = 'dashboard' }: DashboardLayoutProps) {
  ...
  useEffect(() => {
    ...
    // Use query without orderBy for now (index may still be building)
    // We'll sort client-side. Once index is ready, you can add orderBy back
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        const newNotifications = sortedChanges
          ...
          .map(change => {
            ...
            return {
              ...
              timestamp: ticket.createdAt?.toDate ? ticket.createdAt.toDate() : 
                        (ticket.createdAt?.seconds ? new Date(ticket.createdAt.seconds * 1000) : new Date()),
              read: false,
              type: 'attendee',
            };
          });
      ...
      (error: any) => {
        ...
        if (error.code === 'failed-precondition' && error.message?.includes('index')) {
          console.warn('Firestore index is still building. Notifications will work once the index is ready.');
          console.warn('Check index status: https://console.firebase.google.com/project/mobi-c064c/firestore/indexes');
        } else {
