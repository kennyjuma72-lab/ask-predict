...
import { collection, query, where, onSnapshot, updateDoc, doc, getDoc, Timestamp, getDocs } from 'firebase/firestore';
...
function EventAttendeesContent() {
      ...
      for (const ticketDoc of snapshot.docs) {
        ...
        try {
          ...
          attendeesData.push({
            ...
            confirmedAt: ticketData.confirmedAt?.toDate(),
            ticketType: ticketData.ticketType || 'Standard',
            checkInStatus: ticketData.checkInStatus || 'not-checked-in',
            checkInTime: ticketData.checkInTime?.toDate(),
            userInfo: {
  ...
  const handlePaymentConfirmation = async () => {
    ...
    try {
      await updateDoc(doc(db, 'tickets', selectedAttendee.id), {
        status: 'confirmed',
  ...
  const handleCheckIn = async (attendee: Attendee) => {
    try {
      await updateDoc(doc(db, 'tickets', attendee.id), {
        checkInStatus: attendee.checkInStatus === 'checked-in' ? 'not-checked-in' : 'checked-in',
        checkInTime: attendee.checkInStatus !== 'checked-in' ? Timestamp.now() : null,
      });
    } catch (error) {
  ...
  const handleBulkConfirm = async () => {
    try {
      ...
      const updatePromises = Array.from(selectedAttendees).map((attendeeId) =>
        updateDoc(doc(db, 'tickets', attendeeId), {
          status: 'confirmed',
          confirmedAt: Timestamp.now(),
        })
      );
