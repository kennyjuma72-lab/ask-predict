...
function QRScannerContent() {
    ...
    const loadStats = async () => {
        try {
            const ticketsRef = await getDoc(doc(db, 'events', eventId));
            // This is a simplified version - you'd want to query tickets collection
            setStats({
                totalTickets: 0,
                checkedIn: 0,
                pending: 0,
    ...
    const validateTicket = async (qrDataString: string) => {
        try {
            ...
            // Validate QR timestamp (prevent old QR codes)
    ...
    const handleCheckIn = async () => {
        ...
        try {
            await updateDoc(doc(db, 'tickets', validationResult.ticket.id), {
                checkInStatus: 'checked-in',
                checkInTime: Timestamp.now(),
            });

            alert(`✅ ${validationResult.ticket.userName || 'Attendee'} checked in successfully!`);
            loadStats();
            handleDismiss();
        } catch (error) {
                ...
                {showResult && validationResult && (
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                        <div className="bg-white rounded-3xl max-w-md w-full p-8">
                            <div
                                className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 ${validationResult.valid
                            ...
                            <p className="text-center text-slate-600 mb-6 font-semibold text-lg">
                            ...
                            {validationResult.valid && validationResult.ticket && (
                                <div className="bg-gradient-to-br from-slate-50 to-blue-50 rounded-2xl p-6 mb-6 space-y-3">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-bold text-slate-900">{validationResult.ticket.userName || 'Unknown'}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-slate-600 font-semibold">Email:</span>
                                        <span className="font-bold text-slate-900 text-sm">{validationResult.ticket.userEmail || 'N/A'}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-slate-600 font-semibold">Ticket Type:</span>
                                        <span className="font-bold text-slate-900">{validationResult.ticket.ticketType || 'General'}</span>
                                    </div>
                                </div>
                            )}
