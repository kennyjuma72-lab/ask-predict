...
function EditEventContent({ userProfile, eventId, router }: { userProfile: any; eventId: string; router: any }) {
    ...
    const load = async () => {
      ...
      try {
        ...
        setEventData({
          ...
          startAt: data.startAt?.toDate ? data.startAt.toDate() : (data.startAt ? new Date(data.startAt) : new Date()),
          endAt: data.endAt?.toDate ? data.endAt.toDate() : (data.endAt ? new Date(data.endAt) : new Date()),
          ...
          priceText: data.priceOptions?.[0]?.price ? String(data.priceOptions[0].price) : '',
          registrationUrl: data.registrationUrl || '',
          speakers: Array.isArray(data.speakers) ? data.speakers : [],
          agenda: Array.isArray(data.agenda) ? data.agenda.map((it: any) => ({ ...it, time: it.time?.toDate ? it.time.toDate() : (it.time ? new Date(it.time) : new Date()) })) : [],
  ...
  const uploadFile = async (file: File, path: string) => {
    setUploading(true);
    try {
      const fileRef = ref(storage, `events/${eventId}/${path}`);
  ...
  const save = async () => {
    ...
    try {
      ...
      const payload: any = {
        ...
        isActive: true, // Ensure event is active
        type: eventData.type,
        multiDay: eventData.multiDay,
        startAt: Timestamp.fromDate(eventData.startAt),
        endAt: Timestamp.fromDate(eventData.endAt),
        date: Timestamp.fromDate(eventData.startAt), // For backward compatibility
        // Build venue object with address details
        ...
        location: venueName, // String format for mobile app compatibility
        // Capacity - support both field names for compatibility
        capacity: capacityValue,
        maxAttendees: capacityValue, // Mobile app expects this field
        // Price - support both formats
        price: priceValue,
        currency: 'USD',
        priceOptions: eventData.priceText ? [{ id: 'default', name: 'General', price: priceValue, currency: 'USD', isAvailable: true }] : [],
        // Category and tags
        category: eventData.type || 'general',
        tags: [],
        // Organizer info
        organizerId: userProfile?.id,
        organizerName: userProfile?.name || userProfile?.email || 'Event Organizer',
        registrationUrl: eventData.registrationUrl || null,
        speakers: eventData.speakers.map(s => ({ ...s })),
        agenda: eventData.agenda.map(a => ({
          ...a,
          time: Timestamp.fromDate(a.time)
        })),
        partners: eventData.partners.map(p => ({ ...p })),
        // Note: currentAttendees is maintained separately and should not be overwritten
      };
