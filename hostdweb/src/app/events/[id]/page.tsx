...
export default function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isEventOwner = userProfile?.id === event.hostId;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="bg-white/80 backdrop-blur-xl rounded-3xl border border-slate-200/50 shadow-2xl p-8 animate-[fadeIn_0.5s_ease-out_0.2s_backwards]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {event.speakers.map((speaker: any, idx: number) => (
                        <div
                          key={idx}
                          className="group bg-gradient-to-br from-slate-50 to-blue-50 rounded-2xl p-6 border-2 border-slate-200 hover:border-purple-300 hover:shadow-xl transition-all duration-300"
                        >
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                              ...
                              {speaker.company && (
                                <p className="text-sm text-slate-600 font-medium mb-2">{speaker.company}</p>
                              )}
                              {speaker.bio && (
                                <p className="text-sm text-slate-600 font-medium">{speaker.bio}</p>
                              )}
                            </div>
                          </div>
                        </div>
                ...
                {event.agenda && event.agenda.length > 0 && (
                  <div className="bg-white/80 backdrop-blur-xl rounded-3xl border border-slate-200/50 shadow-2xl p-8 animate-[fadeIn_0.5s_ease-out_0.3s_backwards]">
                    <h2 className="text-2xl font-black text-slate-900 mb-6 flex items-center gap-3">
                      <span className="w-10 h-10 bg-gradient-to-br from-emerald-600 to-teal-600 rounded-xl flex items-center justify-center text-white">
                        📅
                      </span>
                      Event Agenda
                    </h2>
                    <div className="space-y-4">
                      {event.agenda.map((item: any, idx: number) => {
                        const time = item.time?.toDate ? item.time.toDate() : new Date(item.time);
                        return (
                          <div
                            key={idx}
                            className="group flex gap-6 p-6 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-2xl border-2 border-emerald-200 hover:border-emerald-300 hover:shadow-lg transition-all duration-300"
                          >
                            <div className="flex-shrink-0">
                              <div className="w-20 h-20 bg-gradient-to-br from-emerald-600 to-teal-600 rounded-xl flex flex-col items-center justify-center text-white shadow-lg">
                                ...
                                {item.duration && (
                                  <span className="text-xs font-bold opacity-90">{item.duration}</span>
                                )}
                              </div>
                            </div>
                            <div className="flex-1">
                              ...
                              {item.speaker && (
                                <p className="text-sm font-bold text-emerald-700 mb-2">👤 {item.speaker}</p>
                              )}
                              {item.description && (
                ...
                {event.partners && event.partners.length > 0 && (
                  <div className="bg-white/80 backdrop-blur-xl rounded-3xl border border-slate-200/50 shadow-2xl p-8 animate-[fadeIn_0.5s_ease-out_0.4s_backwards]">
                    <h2 className="text-2xl font-black text-slate-900 mb-6 flex items-center gap-3">
                      <span className="w-10 h-10 bg-gradient-to-br from-amber-600 to-orange-600 rounded-xl flex items-center justify-center text-white">
                        🤝
                      </span>
                      Event Partners
                    </h2>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {event.partners.map((partner: any, idx: number) => (
                        ...
                        >
                          ...
                          <p className="text-center text-sm font-black text-slate-900 group-hover:text-amber-600 transition-colors">
                            {partner.name}
                          </p>
                        </a>
                      ))}
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white/80 backdrop-blur-xl rounded-3xl border border-slate-200/50 shadow-2xl p-8 sticky top-8 animate-[fadeIn_0.5s_ease-out_0.1s_backwards]">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="pb-6 border-b-2 border-slate-100">
                      <div className="flex items-start gap-4">
                        <div className="w-12 h-12 bg-gradient-to-br from-blue-600 to-cyan-600 rounded-xl flex items-center justify-center text-white text-xl shadow-lg flex-shrink-0">
                          📆
                        </div>
                        <div>
                      ...
                      {event.multiDay && event.endAt && (
                        <div className="flex items-start gap-4 mt-4">
                          <div className="w-12 h-12 bg-gradient-to-br from-purple-600 to-pink-600 rounded-xl flex items-center justify-center text-white text-xl shadow-lg flex-shrink-0">
                            🏁
                          </div>
                          <div>
                            <p className="text-xs font-black text-slate-500 uppercase tracking-wide mb-1">End Date</p>
                            <p className="text-sm font-bold text-slate-900">{formatDate(event.endAt)}</p>
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    {event.venue && (
                      <div className="pb-6 border-b-2 border-slate-100">
                        <div className="flex items-start gap-4">
                          <div className="w-12 h-12 bg-gradient-to-br from-emerald-600 to-teal-600 rounded-xl flex items-center justify-center text-white text-xl shadow-lg flex-shrink-0">
                            📍
                          </div>
                          <div>
                    ...
                    {event.capacity && (
                      <div className="pb-6 border-b-2 border-slate-100">
                        <div className="flex items-start gap-4">
                          <div className="w-12 h-12 bg-gradient-to-br from-violet-600 to-purple-600 rounded-xl flex items-center justify-center text-white text-xl shadow-lg flex-shrink-0">
                            👥
                          </div>
                          <div>
                    ...
                    {event.priceOptions && event.priceOptions.length > 0 && (
                      <div className="pb-6 border-b-2 border-slate-100">
                        <div className="flex items-start gap-4">
                          <div className="w-12 h-12 bg-gradient-to-br from-amber-600 to-orange-600 rounded-xl flex items-center justify-center text-white text-xl shadow-lg flex-shrink-0">
                            💰
                          </div>
                          <div>
                            <p className="text-xs font-black text-slate-500 uppercase tracking-wide mb-1">Price</p>
                            <p className="text-sm font-bold text-slate-900">
                              {event.priceOptions[0].price === 0 ? 'Free' : `$${event.priceOptions[0].price}`}
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                <div className="bg-white/80 backdrop-blur-xl rounded-3xl border border-slate-200/50 shadow-2xl p-8 animate-[fadeIn_0.5s_ease-out_0.2s_backwards]">
                  ...
                  <div className="flex gap-3">
                    <button className="flex-1 p-4 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all duration-300 hover:scale-105 font-bold">
                      Twitter
                    </button>
                    <button className="flex-1 p-4 bg-blue-800 text-white rounded-xl hover:bg-blue-900 transition-all duration-300 hover:scale-105 font-bold">
                      Facebook
                    </button>
                    <button className="flex-1 p-4 bg-green-600 text-white rounded-xl hover:bg-green-700 transition-all duration-300 hover:scale-105 font-bold">
                      WhatsApp
                    </button>
                  </div>
                </div>
