...
function CreateEventContent({ userProfile, router }: { userProfile: any; router: any }) {
  ...
  const handleFileUpload = async (file: File, path: string) => {
    try {
      setUploading(true);
      const storageRef = ref(storage, `events/${Date.now()}/${path}`);
  ...
  const createEvent = async () => {
    if (!validateForm()) {
      alert('Please fix the errors before creating the event');
    ...
    try {
      ...
      const payload = {
        ...
        // Venue and location - support both formats for compatibility
        venue: venueObject,
        ...
        price: priceValue,
        currency: 'USD',
        priceOptions: eventData.priceText ? [{
          id: 'default',
          name: 'General',
          price: priceValue,
          currency: 'USD',
        ...
        // Attendee tracking
        currentAttendees: 0, // Initialize to 0
        // Category and tags
      ...
      logger.logEventOperation('event_created', eventRef.id, {
        title: eventData.title,
        hostId: userProfile.id,
        duration,
      });
      logger.logDatabaseOperation('create', 'events', { eventId: eventRef.id });
      logger.logPerformance('create_event', duration, { eventId: eventRef.id });
    ...
    try {
      ...
      const payload = {
        ...
        priceOptions: eventData.priceText ? [{
          id: 'default',
          name: 'General',
          price: Number(eventData.priceText.replace(/[^0-9.]/g, '')),
          currency: 'USD',
          isAvailable: true
        }] : [],
        registrationUrl: eventData.registrationUrl || null,
      ...
      logger.logEventOperation('event_saved_draft', eventRef.id, {
        title: payload.title,
        hostId: userProfile.id,
        duration,
      });
      logger.logDatabaseOperation('create', 'events', { eventId: eventRef.id });
      logger.logPerformance('save_draft', duration, { eventId: eventRef.id });
