    ...
    const loadAnalyticsData = async () => {
      try {
        ...
        const filteredEventsData = eventsData.filter(event => {
          const eventDate = event.startAt?.toDate ? event.startAt.toDate() : (event.startAt?.seconds ? new Date(event.startAt.seconds * 1000) : new Date(event.startAt));
        ...
        for (const event of filteredEventsData) {
          const startDate = event.startAt?.toDate ? event.startAt.toDate() : (event.startAt?.seconds ? new Date(event.startAt.seconds * 1000) : new Date(event.startAt));
          
          const attendeeCount = event.currentAttendees || 0;
          const eventRevenue = attendeeCount * (event.priceOptions?.[0]?.price || 0);
