...
function Sidebar({ currentPage, className = '' }: { currentPage: string; className?: string }) {
  const { userProfile } = useAuth();

  const menuItems = [
    { id: 'dashboard', label: 'Dashboard', icon: '📊', path: '/dashboard' },
    { id: 'events', label: 'Events', icon: '🎫', path: '/events' },
    { id: 'attendees', label: 'Attendees', icon: '👥', path: '/attendees' },
    { id: 'analytics', label: 'Analytics', icon: '📈', path: '/analytics' },
    { id: 'settings', label: 'Settings', icon: '⚙️', path: '/settings' },
  ];

  return (
    <div className={`bg-white border-r-2 border-pink-100 shadow-xl z-50 flex flex-col ${className}`}>
      {/* Logo */}
      <div className="p-6 border-b-2 border-pink-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-purple-900">FemVents</h1>
            <p className="text-xs text-purple-500">Event Management</p>
          </div>
        </div>
      </div>
          ...
          <a
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all duration-300 group ${
              currentPage === item.id
                ...
                : 'text-purple-600 hover:bg-pink-50 hover:text-purple-900'
            }`}
          >
      ...
      <div className="p-4 border-t-2 border-pink-100">
        <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-pink-50 transition-all duration-300 cursor-pointer group">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-purple-500">
              {userProfile?.role === 'host' ? 'Event Host' : (userProfile?.role || 'User')}
            </p>
          </div>
  ...
  if (!mounted) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-rose-50 relative overflow-hidden">
        <Sidebar currentPage="dashboard" className="hidden lg:flex fixed left-0 top-0 h-full w-64" />
        <div className="lg:ml-64 ml-0 p-4 sm:p-6 lg:p-8 relative z-10">
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute left-0 top-0 h-full w-64">
            <Sidebar currentPage="dashboard" className="flex h-full w-64" />
          </div>
        </div>
      )}
      ...
      <div className="lg:ml-64 ml-0 p-4 sm:p-6 lg:p-8 relative z-10">
        ...
        <div className="mb-12 animate-[fadeIn_0.8s_ease-out]">
          <div className="flex items-center justify-between flex-wrap gap-6">
            {/* Mobile menu button */}
            <button
              className="lg:hidden inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-pink-200 text-purple-700 bg-white shadow-sm active:scale-95 transition"
