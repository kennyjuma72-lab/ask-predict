...
import { Breadcrumb, Card, CardHeader, CardBody } from '../components';
...
export default function BackupExportPage() {
    ...
    const collections = [
        ...
        { name: 'contentReports', label: 'Content Reports', icon: '🚩' },
        { name: 'eventTemplates', label: 'Event Templates', icon: '📋' },
        { name: 'platformSettings', label: 'Platform Settings', icon: '⚙️' },
    ];
    ...
    const exportCollection = async (collectionName: string, format: 'csv' | 'json') => {
        ...
        try {
            ...
            if (data.length === 0) {
                toast(`No data found in ${collectionName}`, 'warning');
                return;
            }
            ...
            toast(`Failed to export ${collectionName}`, 'error');
        } finally {
            setExporting(false);
        }
    };
    ...
    const exportAll = async () => {
        ...
        toast('All collections exported!', 'success');
        setExporting(false);
    };
    ...
    const exportGDPR = async () => {
        ...
        try {
            ...
            for (const coll of collections) {
                ...
                userData[coll.name] = snapshot.docs
            ...
            toast('GDPR export complete!', 'success');
        } catch (error) {
    ...
    return (
        <AdminLayout>
            <div className="space-y-6">
                {/* Breadcrumb */}
                <Breadcrumb items={[{ label: 'Backup & Export' }]} />
