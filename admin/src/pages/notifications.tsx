...
import { Card, CardHeader, CardBody, Breadcrumb, Button } from '../components';
    ...
    const handleSend = async (scheduled: boolean = false) => {
        ...
        try {
            const scheduledFor = scheduled && formData.scheduleDate && formData.scheduleTime
                ? Timestamp.fromDate(new Date(`${formData.scheduleDate}T${formData.scheduleTime}`))
                : null;
            ...
            const notification = {
                ...
                scheduledFor,
                sentAt: !scheduled ? Timestamp.now() : null,
                createdBy: 'admin',
            ...
            await addDoc(collection(db, 'platformNotifications'), notification);
                                    ...
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1">
                                            ...
                                            <div className="flex gap-4 text-sm text-gray-500">
                                                ...
                                                <span>
                                                    {notif.sentAt?.seconds
                                                        ? `Sent: ${new Date(notif.sentAt.seconds * 1000).toLocaleString()}`
                                                        : notif.scheduledFor?.seconds
                                                            ? `Scheduled: ${new Date(notif.scheduledFor.seconds * 1000).toLocaleString()}`
                                                            : 'Draft'
                                                    }
                                                </span>
                                            </div>
