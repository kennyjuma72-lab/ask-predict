...
export const EventsScreen: React.FC = () => {
  ...
  const eventsCollectionRef = useRef(collection(db, 'events'));
  ...
  const normalizeEvent = (doc: any) => {
    const data = doc.data();
    const ensureDate = (value: any) => (value?.toDate ? value.toDate() : value);

    return {
      id: doc.id,
      ...data,
      date: ensureDate(data.date) || ensureDate(data.startAt),
      startAt: ensureDate(data.startAt),
      endAt: ensureDate(data.endAt),
      createdAt: ensureDate(data.createdAt),
      updatedAt: ensureDate(data.updatedAt),
      // Field mapping for compatibility
      location: data.location || data.venue?.name || data.venue || 'Online',
      maxAttendees: data.maxAttendees || data.capacity || 0,
      currentAttendees: data.currentAttendees || 0,
      price: data.price || (data.priceOptions?.[0]?.price || 0),
      currency: data.currency || data.priceOptions?.[0]?.currency || 'USD',
      category: data.category || data.type || 'general',
      tags: data.tags || [],
      organizerName: data.organizerName || 'Event Organizer',
      isActive: data.isActive !== undefined ? data.isActive : (data.isPublished !== false),
    };
  };

  useEffect(() => {
    const unsubscribe = onSnapshot(
      eventsCollectionRef.current,
      snapshot => {
        const normalized = snapshot.docs.map(normalizeEvent);
        normalized.sort((a, b) => {
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    try {
      const snapshot = await getDocs(eventsCollectionRef.current);
      const normalized = snapshot.docs.map(normalizeEvent);
      normalized.sort((a, b) => {
        const aDate = a.date ? new Date(a.date).getTime() : Number.MAX_SAFE_INTEGER;
        const bDate = b.date ? new Date(b.date).getTime() : Number.MAX_SAFE_INTEGER;
