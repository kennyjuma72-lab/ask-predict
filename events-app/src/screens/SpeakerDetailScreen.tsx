      ...
      try {
        ...
        const unsubscribe = onSnapshot(
          ...
          (eventDoc) => {
            try {
              if (eventDoc.exists()) {
                ...
                if (speakerId && eventData.speakers && Array.isArray(eventData.speakers)) {
                  const foundSpeaker = eventData.speakers.find((s: any, index: number) => 
          ...
          (error) => {
            ...
            // Try fallback to one-time fetch
            getDoc(eventDocRef)
              .then((eventDoc) => {
                if (eventDoc.exists()) {
                  const eventData = eventDoc.data();
