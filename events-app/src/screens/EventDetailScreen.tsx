...
import { Button, Badge, SkeletonCard, SkeletonText } from '../components';
...
export default function EventDetailScreen({ route, navigation }: EventDetailScreenProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (eventDoc) => {
        if (eventDoc.exists()) {
          ...
          const updatedEvent = {
            ...
            date: eventData.date?.toDate() || eventData.startAt?.toDate(),
            startAt: eventData.startAt?.toDate(),
            endAt: eventData.endAt?.toDate(),
            // Field mapping for compatibility
            location: eventData.location || eventData.venue?.name || eventData.venue || 'Online',
            maxAttendees: eventData.maxAttendees || eventData.capacity || 0,
            currentAttendees: eventData.currentAttendees || 0,
            price: eventData.price || (eventData.priceOptions?.[0]?.price || 0),
            currency: eventData.currency || eventData.priceOptions?.[0]?.currency || 'USD',
            category: eventData.category || eventData.type || 'general',
            tags: eventData.tags || [],
            organizerName: eventData.organizerName || 'Event Organizer',
            isActive: eventData.isActive !== undefined ? eventData.isActive : (eventData.isPublished !== false),
          };
          setEvent(updatedEvent);
          setAttendeeCount({
            total: eventData.currentAttendees || 0,
            confirmed: eventData.currentAttendees || 0,
            pending: 0,
          });
      ...
      (error) => {
        ...
        // Try fallback to one-time fetch if real-time fails
        getDoc(eventDocRef)
          .then((eventDoc) => {
            if (eventDoc.exists()) {
              const eventData = eventDoc.data();
          ...
          .catch((fallbackError) => {
            ...
            if (fallbackError.code === 'unavailable') {
              errorMessage = 'No internet connection. Please check your network.';
            } else if (fallbackError.message) {
              errorMessage = `Failed to load event: ${fallbackError.message}`;
            }
  ...
  const handleRegister = async () => {
    ...
    if (!event?.id) {
      showToast('Event information is not available', 'error');
      return;
    }
