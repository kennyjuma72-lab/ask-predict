...
export default function SponsorScreen({ navigation, route }: SponsorScreenProps) {
        ...
        (eventDoc) => {
          try {
            if (eventDoc.exists()) {
              ...
              // Use event's partners data if available, otherwise use sample data
              if (eventData.partners && Array.isArray(eventData.partners) && eventData.partners.length > 0) {
                // Map partners to sponsor format, ensuring they have required fields
                const mappedPartners: Sponsor[] = eventData.partners.map((partner: any, index: number) => ({
              ...
              // Event not found, use sample data
              setSponsors(sampleSponsors);
              setLoading(false);
            }
          } catch (dataError) {
  ...
  const handleWebsitePress = (sponsor: Sponsor) => {
    if (sponsor.website) {
      showToast(`Would open ${sponsor.website} in browser`, 'info');
    }
  };

  const handleContactPress = (sponsor: Sponsor) => {
    if (sponsor.contact) {
      showToast(`Would open email to ${sponsor.contact}`, 'info');
    }
  };
      ...
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        <View style={styles.headerContent}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => {
              showToast('Our valued partners and sponsors', 'info');
            }}
          >
