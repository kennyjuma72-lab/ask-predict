...
export default function ScheduleScreen({ navigation, route }: ScheduleScreenProps) {
  ...
  useEffect(() => {
    ...
    try {
      const eventDocRef = doc(db, 'events', route.params.eventId);
           ...
           if (eventDoc.exists()) {
             ...
             setEventData({
               ...
               createdAt: data.createdAt?.toDate(),
                // Ensure agenda items have proper time conversion
                agenda: data.agenda?.map((item: any) => ({
                  ...item,
                  time: item.time?.toDate ? item.time.toDate() : (item.time instanceof Date ? item.time : item.time),
                })) || [],
              });
              setLoading(false);
            } else {
      ...
      Alert.alert('Error', 'Failed to connect to event data');
      setLoading(false);
    }
  ...
  const toggleFavorite = (sessionId: string) => {
    ...
    if (newFavorites.has(sessionId)) {
      ...
      showToast('Session removed from favorites', 'info');
    } else {
      newFavorites.add(sessionId);
      showToast('Session added to favorites', 'success');
    }
  ...
  const handleAddToCalendar = (session: Session) => {
    showToast(`"${session.title}" added to your calendar`, 'success');
  };
