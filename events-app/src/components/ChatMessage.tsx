...
export default function ChatMessage({ message, isOwnMessage, currentUserId, onReply, collectionName }: ChatMessageProps) {
  ...
  const handleReaction = async (emoji: string) => {
    try {
      const messageRef = doc(db, collectionName, message.id);
      ...
      if (userReaction && userReaction.emoji === emoji) {
        ...
        await updateDoc(messageRef, {
          reactions: existingReactions.filter(r => !(r.userId === currentUserId && r.emoji === emoji))
        });
      } else {
        ...
        await updateDoc(messageRef, {
          reactions: arrayUnion({
            ...
            userName: 'You' // Will be replaced with actual name from context
          })
        });
      }
